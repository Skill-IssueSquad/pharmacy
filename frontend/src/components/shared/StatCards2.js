...
const StatCards2 = () => {
  ...
  const [showProgressPharmacist, setShowProgressPharmacist] = useState(true);
    ...
    const fetchDataFromDatabase = async () => {
      ...
      try {
        ...
        if(response.ok){
            // Update the state with the fetched data
            const data = json.data
            setPharmacistCount(data.length);
            setShowProgressPharmacist(false);
        }
        } catch (error) {
        console.error('Error fetching data:', error);
        }
        ...
        try {
          // Fetch data from the database
          const response = await fetch('/admin/viewPatientInfo', {credentials: 'include',});
          const json = await response.json();
      ...
      <Grid item xs={12} md={12}>
        <Card elevation={3} sx={{ p: 2 }}>
          <ContentBox>
            ...
            <H3 style={{color:'navy'}}>Pharmacists</H3>
          </ContentBox>

          <ContentBox sx={{ pt: 2 }}>
          {!showProgressPharmacist && <H1>{pharmacistCount}</H1>}
            {showProgressPharmacist && <CircularProgress color="inherit" style={{marginLeft:'110px', marginTop:'10px'}} size={25} />} 
          </ContentBox>
        </Card>
      </Grid>
