...
const MedicineItem = ({ alternative }) => (
  <div style={{ border: '1px solid #ddd', padding: '20px', marginBottom: '20px', display: 'flex', alignItems: 'center', background: '#f8f8f8' }}>
    <div style={{ flex: '1' }}>
      <h2>{alternative.medicineName}</h2>
      <p><strong>Medicinal Usage:</strong> {alternative.medicinalUsage}</p>
      <p><strong>Description:</strong> {alternative.description}</p>
      <ul>
        {alternative.activeIngredients.map((ingredient) => (
          <li key={ingredient._id}>
            <p><strong>Ingredient Name:</strong> {ingredient.ingredientName}</p>
            <p><strong>Ingredient Amount:</strong> {ingredient.ingredientAmount}</p>
          </li>
        ))}
      </ul>
      <p><strong>Price:</strong> {alternative.price}</p>
      <p><strong>Status:</strong> {alternative.quantity > 0 ? 'Available' : 'Out of Stock'}</p>
    </div>
    <div style={{ flex: '1', marginLeft: '20px' }}>
      <img src={alternative.picture} alt={alternative.picture} style={{ width: '100%', maxWidth: '200px', height: 'auto' }} />
    </div>
  </div>
);

const AlternativesView = () => {
  const username = "bahythree";
...
console.log("username:"+ username,"id:"+ id);
  fetch('http://localhost:8001/patient/addAlternative', {
    ...
    .catch((error) => console.error('Error Adding Alternative Medicine to Pharmacy:', error));
    fetchAlternatives();
  }
  ...
  return (
    <div>
      <ResponsiveAppBar />
      <h1 style={{ textAlign: 'center', marginBottom: '20px', color: '#333' }}>Alternative Medicines for {medicineName}</h1>
      {alternatives.map((alternative) => (
        <div key={alternative.id} style={{ marginBottom: '20px' }}>
          <MedicineItem alternative={alternative} />
          <div style={{ textAlign: 'left', margin: '10px 0' }}>
          <button  onClick={() => addToCartA(alternative._id,alternative.medicineName,alternative.requiresPrescription)} style={{ marginBottom: '3px', marginTop: '3px' }}>Add to Cart</button>
          </div>
        </div>
      ))}
    </div>
