...
const ViewInfo = ()=> {
    ...
    const handleUploadDocuments = async () => {
        try {
            ...
            const response = await axios.patch('http://localhost:8000/pharmacistRequest/updateInfo/' + username, formData, {
    ...
    useEffect(() => {
        const getInfo = async () => {
            try {
                console.log(username);
                const response = await fetch('http://localhost:8000/pharmacistRequest/viewInfo' , {method: 'POST', 
                headers: {
