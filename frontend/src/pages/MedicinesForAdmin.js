...
const MedicinesForAdmin = () => {
  ...
  const filteredByMedicalUsage = filteredMedicines.filter((medicine) => {
    if (filterMedicalUsage) {
      ...
      return medicinalUsage && medicinalUsage.trim()
        ? medicinalUsage.toLowerCase().includes(filter)
        : false;
    }
    return true;
  });
...
export default MedicinesForAdmin;
